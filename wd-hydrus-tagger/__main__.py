...
hash list or by searching Hydrus tags.
...
def get_file_with_retry(client: hydrus_api.Client, file_hash: str, retries: int = 5, delay: int = 5) -> Any:
    """Fetch a file from Hydrus with retry semantics.

    Attempts to fetch ``file_hash`` from the provided Hydrus ``client`` up to
    ``retries`` times, sleeping ``delay`` seconds between transient failures.

    :param client: Hydrus API client used to fetch files.
    :param file_hash: The Hydrus file hash to request.
    ...
    :raises APIError: If the Hydrus API indicates a missing file or another
...
@click.group()
def cli():
    """Click command group for the WD Hydrus Tagger CLI.

    This function serves as the top-level Click entrypoint grouping subcommands
    for the utility. It is intentionally empty; Click attaches commands to
    this group via `cli.add_command` at module import time.
    """
    pass
...
def evaluate_api_batch(hashfile: Optional[str], search_tag: tuple[str, ...], token: str, cpu: bool,
    ...
    """Evaluate a batch of files from Hydrus and apply AI-generated tags.

    This command entrypoint can read a list of hashes from ``hashfile`` or
    search Hydrus by tags provided in ``search_tag``. It loads model metadata
    from the selected model folder, instantiates the local tagger, queries the
    Hydrus API for files, interrogates images to produce tags and ratings, and
    writes tags back to the specified Hydrus tag service.

    Side effects:
    - writes log files to the ``logs`` directory
    - may append to ``done-hashes-<model>.txt`` and ``bad-hashes.txt``
    - makes network requests to the Hydrus server
